...
public class SearchClient {
    ...
    // Behavior: 
    //   - This method makes a map consisting of words and their corresponding medias that
    //   - have the specific word in it, considering these words case-insensitively.
    // Parameters:
    //   - documents: list of media objects (books)
    // Returns:
    //   - Map<String, Set<Media>>: the map that maps words and media objects that contain those
    //   words.
    public static Map<String, Set<Media>> createIndex(List<Media> documents) {
        ...
        for(Media media : documents){
            List<String> words = media.getContent();
    ...
    // Behavior: 
    //   - This method finds possible medias based on the user's search
    // Parameters:
    //   - index: the map that maps medias to words
    //   - query: the user's search
    // Returns:
    //   - Set<Media>: the possible medias that the user's search could lead to, ordered from most
    //   ratings to least ratings.
    public static Set<Media> search(Map<String, Set<Media>> index, String query) {
        ...
        while(scan.hasNext()){
            ...
            if(index.containsKey(word)){
                for(Media media : index.get(word)){
                    answer.add(media);
                }
            }
        }
